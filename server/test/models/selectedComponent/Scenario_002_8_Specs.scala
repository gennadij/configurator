...
class Scenario_002_8_Specs extends Specification with MessageHandler with BeforeAfterAll{
  ...
  "Specification spezifiziert der NextStep der Konfiguration" >> {
    "S1 -> C1, C2, C3" >> {
      ...
      (result_2 \ "step" \ JsonKey.nameToShow).asOpt[String] === Some("S1")
      (result_2 \ "step" \ "components").asOpt[List[JsValue]].get.size === 2
      ((result_2 \ "step" \ "components")(0) \ JsonKey.nameToShow).asOpt[String] === Some("C12")
      ((result_2 \ "step" \ "components")(1) \ JsonKey.nameToShow).asOpt[String] === Some("C11")
      ...
      (result_3 \ "step" \ JsonKey.nameToShow).asOpt[String] === Some("S1")
      (result_3 \ "step" \ "components").asOpt[List[JsValue]].get.size === 2
      ((result_3 \ "step" \ "components")(0) \ JsonKey.nameToShow).asOpt[String] === Some("C12")
      ((result_3 \ "step" \ "components")(1) \ JsonKey.nameToShow).asOpt[String] === Some("C11")
    }
  }
}