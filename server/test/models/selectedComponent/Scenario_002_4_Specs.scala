...
class Scenario_002_4_Specs  extends Specification with MessageHandler with BeforeAfterAll{
  ...
  "Specification spezifiziert CurrentConfig bei der doppeltem Auswahl der Komponente " >> {
    "Die Komponente C11, C12, C12 wird ausgewaelt" >> {
      ...
      //TODO ExcludeComponentInternal
      (componentOut_3 \ JsonKey.result \ JsonKey.warning \ JsonKey.excludeComponentExternal \ JsonKey.name).asOpt[String] ===
        Some(ExcludeComponentExternal().name)
      ...
      val jsonCurrentConfigOut_3: JsValue = wC.handleMessage(jsonCurrentConfigIn_2, wC.currentConfigContainerBO)
