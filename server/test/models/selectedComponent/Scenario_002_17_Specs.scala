...
class Scenario_002_17_Specs extends Specification with MessageHandler with BeforeAfterAll{
  ...
  "Specification for scenario 17" >> {
    "S1 -> C1, C2 | S2 -> C2" >> {
      ...
      (jsonNextStepOut \ JsonKey.result \ JsonKey.step \ "nameToShow").asOpt[String].get === "S2"
      (jsonNextStepOut \ JsonKey.result \ JsonKey.step \ "components").asOpt[Set[JsValue]].get.size === 2
      (((jsonNextStepOut \ JsonKey.result \ JsonKey.step \ "components")(0)) \ "nameToShow").asOpt[String].get === "C21"
      (((jsonNextStepOut \ JsonKey.result \ JsonKey.step \ "components")(1)) \ "nameToShow").asOpt[String].get === "C22"
      (jsonNextStepOut \ JsonKey.result \ "status" \ "firstStep").asOpt[String] === None
      (jsonNextStepOut \ JsonKey.result \ "status" \ "nextStep" \ "status").asOpt[String].get === "NEXT_STEP_EXIST"
      (jsonNextStepOut \ JsonKey.result \ "status" \ "fatherStep").asOpt[String] === None
      (jsonNextStepOut \ JsonKey.result \ "status" \ "common" \ "status").asOpt[String].get === "SUCCESS"
      
      val componentId22: String = (jsonNextStepOut \ JsonKey.result \ JsonKey.step \ "components").asOpt[List[JsValue]].get
      ...
      (componentOut4_22 \ JsonKey.result \ "status" \"componentType" \ "status").asOpt[String].get === "DEFAULT_COMPONENT"
      (componentOut4_22 \ JsonKey.result \ "status" \"selectedComponent" \ "status").asOpt[String].get === "ADDED_COMPONENT"
      (componentOut4_22 \ JsonKey.result \ "status" \JsonKey.selectionCriterion \ "status").asOpt[String].get === "REQUIRE_NEXT_STEP"
      (componentOut4_22 \ JsonKey.result \ "status" \JsonKey.excludeDependencyInternal \ "status").asOpt[String] === Some(NotExcludedComponentInternal().status)
      (componentOut4_22 \ JsonKey.result \ "status" \"common" \ "status").asOpt[String].get === "SUCCESS"
      ...
      ((result_5 \ JsonKey.step \ "components")(0) \ "nameToShow").asOpt[String] === Some("C12")
      ((result_5 \ JsonKey.step \ "components")(1) \ "nameToShow").asOpt[String] === Some("C11")
      (result_5 \ JsonKey.step \"nextStep" \ "nameToShow").asOpt[String] === Some("S2")
      (result_5 \ JsonKey.step \"nextStep" \ "components").asOpt[List[JsValue]].get.size === 1
      ((result_5 \ JsonKey.step \ "nextStep" \ "components")(0) \ "nameToShow").asOpt[String] === Some("C22")
    }
  }
}