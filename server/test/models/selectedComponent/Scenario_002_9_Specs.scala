...
class Scenario_002_9_Specs extends Specification with MessageHandler with BeforeAfterAll{
  ...
  "Specification spezifiziert der NextStep der Konfiguration" >> {
    "S1 -> C1, C2, C3, C1" >> {
      ...
      val result_4 = componentOut_4 \ JsonKey.result
      (result_4 \ JsonKey.excludeDependenciesOut).asOpt[List[JsValue]].get.size === 1
      (result_4 \ JsonKey.excludeDependenciesIn).asOpt[List[JsValue]].get.size === 1
      (result_4 \ JsonKey.lastComponent ).asOpt[Boolean].get === false
      (result_4 \ JsonKey.addedComponent ).asOpt[Boolean].get === false
      (result_4 \ JsonKey.info \ JsonKey.selectionCriterion \ JsonKey.name).asOpt[String].get === AllowNextComponent().name
      (result_4 \ JsonKey.result \ JsonKey.warning).asOpt[JsObject] === None
      (result_4 \ JsonKey.errors ).asOpt[JsObject] === None

      val jsonCurrentConfigOut_4: JsValue = CommonFunction.currentCongig(wC)
      
      val result_5 = (jsonCurrentConfigOut_4 \ "result")
      ...
      (result_5 \ JsonKey.step \ JsonKey.components).asOpt[List[JsValue]].get.size === 1
