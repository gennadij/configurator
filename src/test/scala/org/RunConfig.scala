...
class RunConfig extends Specification{
  ...
  def e6 = ConfigMgr.getNextStep(Set("001003")).isInstanceOf[DefaultStep]  must_== true
  
  def e3 = ConfigMgr.getNextStep(Set("003002", "003003")).id must_== "004"
  
  def e7 = ConfigMgr.getNextStep(Set("003002", "003003")).isInstanceOf[DefaultStep]  must_== true
  
  def e4 = ConfigMgr.getNextStep(Set("004003")).id must_== "006"
  
  def e8 = ConfigMgr.getNextStep(Set("004003")).isInstanceOf[LastStep]  must_== true
  
  def e9 = ConfigMgr.getNextStep(Set("006001")).id must_== "0"
  
  def e10 = ConfigMgr.getNextStep(Set("006001")).isInstanceOf[FinalStep]  must_== true
}
